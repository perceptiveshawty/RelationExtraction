...
def main(trained_pipeline: Path, test_data: Path, print_details: bool):
    ...
    has_negatives = str(trained_pipeline).rsplit('/', 1)[0].endswith('negatives') 
    ...
    _score_and_format(random_examples, thresholds, has_negatives)

    print()
    print("Results of the trained model:")
    _score_and_format(examples, thresholds, has_negatives)


def _score_and_format(examples, thresholds, wn):
    if not wn:
        ...
        for threshold in thresholds:
            r = score_relations_with_negs(examples, threshold)
            results = {k: "{:.2f}".format(v * 100) for k, v in r.items()}
            print(f"threshold {'{:.2f}'.format(threshold)} \t {results}")
