...
class RelationExtractor(TrainablePipe):
    ...
    def score(self, examples: Iterable[Example], **kwargs) -> Dict[str, Any]:
        ...
        if not self.name.endswith('negatives'):
            return score_relations(examples, self.threshold)
        else:
            return score_relations_with_negs(examples, self.threshold)
...
def score_relations_with_negs(examples: Iterable[Example], threshold: float) -> Dict[str, Any]:
    """Score a batch of examples."""
    micro_prf = PRFScore()
    for example in examples:
        gold = example.reference._.rel
        ...
        for key, pred_dict in pred.items():
            ...
            for k, v in pred_dict.items():
                if v >= threshold:
                    if k in gold_labels: # TP = found the person and company or some other relation correctly
                        micro_prf.tp += 1
                    elif k == 'PERSON_AND_COMPANY' and k not in gold_labels: # FP = predicted person and company as some other relation
                        micro_prf.fp += 1 # no penalty for predicting other relations incorrectly
                else:
                    if k == 'PERSON_AND_COMPANY' and k in gold_labels: # FN = predicted some other relation instead of person and company
                        micro_prf.fn += 1 # no penalty for predicting other relations incorrectly
    return {
        "rel_micro_p": micro_prf.precision,
        "rel_micro_r": micro_prf.recall,
        "rel_micro_f": micro_prf.fscore,
